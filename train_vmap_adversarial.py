...
from learning_adversarial import sample_batch, batch_epoch
...
def train_once(key, entropy_coeff, clip_epsilon):
    ...
    optimizer_adversarial_representation = optax.chain(optax.clip_by_global_norm(max_norm=max_norm), 
                                           optax.adam(lr))
    optimizer_adversarial_behaviour = optax.chain(optax.clip_by_global_norm(max_norm=max_norm), 
                                           optax.adam(lr))
    
    optimizers = (optimizer_representation, optimizer_behaviour, optimizer_adversarial_representation, optimizer_adversarial_behaviour)
    ...
    optimizer_adversarial_representation_state = optimizer_adversarial_representation.init(model_params)    
    optimizer_adversarial_behaviour_state = optimizer_adversarial_behaviour.init(model_params)

    optimizer_states = (optimizer_representation_state, optimizer_behaviour_state, optimizer_adversarial_representation_state, optimizer_adversarial_behaviour_state)
...
if __name__ == "__main__":
    ...
    # Log to wandb:
    if WANDB:
        wandb.init(project="ppo_baselines_shared_adversarial", 
                   config=config,
