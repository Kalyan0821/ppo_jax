...
def batch_epoch(batch: dict[str, jnp.array],
    ...
    (optimizer_representation, optimizer_behaviour,
    optimizer_adversarial_representation, optimizer_adversarial_behaviour) = optimizers

    (optimizer_representation_state, optimizer_behaviour_state, 
     optimizer_adversarial_representation_state, optimizer_adversarial_behaviour_state) = optimizer_states

    initial_carry = {"model_params": model_params,
                     "adversarial_params": model_params,
                     "optimizer_representation_state": optimizer_representation_state,
                     "optimizer_behaviour_state": optimizer_behaviour_state,
                     "optimizer_adversarial_representation_state": optimizer_adversarial_representation_state,
                     "optimizer_adversarial_behaviour_state": optimizer_adversarial_behaviour_state}
    def scan_function(carry, idx):
        ...
        ########### Update representation w.r.t. adversarial policy ###########
        ...
        (minibatch_loss, loss_info), gradient = val_and_grad_function(carry["adversarial_params"],
                                                                      ...
                                                                      clip_epsilon)
        modified_gradient = unfreeze(gradient)
        assert "logits" in modified_gradient["params"] and "value" in modified_gradient["params"]
        modified_gradient["params"]["logits"] = jax.tree_map(lambda x: jnp.zeros_like(x), gradient["params"]["logits"])
        modified_gradient["params"]["value"] = jax.tree_map(lambda x: jnp.zeros_like(x), gradient["params"]["value"])

        param_updates, carry["optimizer_adversarial_representation_state"] = optimizer_adversarial_representation.update(
                                                                   freeze(modified_gradient),
        ...
        param_updates, carry["optimizer_adversarial_behaviour_state"] = optimizer_adversarial_behaviour.update(
                                                                    freeze(modified_gradient),
    ...
    optimizer_states = (carry["optimizer_representation_state"], carry["optimizer_behaviour_state"], 
                        carry["optimizer_adversarial_representation_state"], carry["optimizer_adversarial_behaviour_state"])
